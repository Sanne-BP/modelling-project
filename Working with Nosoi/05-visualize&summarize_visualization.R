...
# To show the graph, we need to invert column 1 and 2, and discard the first line (i.e. the first host, because it shows "NA" as infecting host). We then provide *Nosoi* output as elements for the vertices (nodes).
...
SimulationA$numID <- as.numeric(as.factor(SimulationA$hosts.ID)) 
#gets a unique numeric ID for each host
SimulationA[is.na(out.time),out.time:=(test.nosoiA$total.time)] 
#active hosts get a "end time" (i.e. the end of the simulation)
...
#--------------------------Epidemic: Spread in discrete structured populations
...
test2 <- results |>
  group_by(time,state) |>
  summarise(N=length(hosts.ID)) |>
  left_join(layout,by=c("state"="name"))
...
animate(animated.plot, nframes=test.nosoiA$total.time*2+10,duration=40,end_pause=10,fps = 25)
...
#--------------------------Epidemic: Spread in structured populations in continuous space
